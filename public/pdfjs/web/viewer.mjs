...
class Toolbar {
  ...
  constructor(options, eventBus, toolbarDensity = 0) {
    ...
    const buttons = [{
      ...
      element: options.showDocumentListButton,
      eventName: null,
      onClick: () => {
        const targetOrigin = window.location.ancestorOrigins?.[0] || document.referrer;
        if (targetOrigin) {
          window.parent.postMessage({
            type: 'pdfjs-custom-show-document-list-button-clicked'
          }, targetOrigin);
        }
      }
    }, {
...
function getViewerConfiguration() {
  return {
    ...
    toolbar: {
      ...
      showDocumentListButton: document.getElementById("showDocumentListButton"),
